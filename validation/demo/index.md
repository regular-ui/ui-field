...
- `isRequired`表示该值不能为空
- `isFilled`表示该值首尾去除空白字符`trim`之后也不能为空
- 当类型为`is`时，需要给`options`传一个正则表达式，根据此正则表达式来验证该值
- 当类型为`isNot`时，与`is`同理，只是验证结果为正则表达式的否定值
- 当类型为`method`时，需要给`options`传一个同步方法，根据这个方法的计算结果来验证该值
- 当类型为`async`时，需要给`options`传一个异步方法，异步方法中需要调用`done`回调函数，根据这个回调结果来是否验证该值
...
<form class="m-form">
    <div class="form_item">
        ...
        <span class="form_field">
            <inputField rules={rules[0]} maxlength=12 placeholder="试一试什么都不要输入" />
        </span>
    </div>
    <div class="form_item">
        <label class="form_label">isFilled<span class="form_required">*</span>：</label>
        <span class="form_field">
            <inputField rules={rules[1]} maxlength=12 placeholder="试一试输入空格" />
        </span>
    </div>
    <div class="form_item">
        <label class="form_label">is：</label>
        <span class="form_field">
            <inputField rules={rules[2]} maxlength=12 placeholder="以数字开头" />
        </span>
    </div>
    <div class="form_item">
        <label class="form_label">isNot：</label>
        <span class="form_field">
            <inputField rules={rules[3]} maxlength=12 placeholder="不能以数字开头" />
        </span>
    </div>
    <div class="form_item">
        <label class="form_label">method：</label>
        <span class="form_field">
            <inputField rules={rules[4]} maxlength=12 placeholder="验证码为6543" />
        </span>
    </div>
    <div class="form_item">
        <label class="form_label">async：</label>
        <span class="form_field">
            <inputField rules={rules[5]} maxlength=12 placeholder="用户名user已被使用" />
        </span>
    </div>
</form>
```
...
表单验证行为按照实时性通常可以分为三种：提交验证、失焦验证、实时验证，分别对应验证规则的三种触发方式：`submit`, `blur`, `input`，规则中默认为`submit`。
...
let component = new RGUI.Component({
    ...
    submit() {
        this.$refs.validation.$one((conclusion) => {
...
在表单控件的值实时改变时，对该控件进行验证，通常对应`<input>`控件的`input`事件，其他表单控件视情况而定。在Regular UI的验证规则中，`on`字段中包含`input`可以激活此种验证。当只激活实时验证时，失焦验证会跳过此规则并且覆盖原来的结果，因此我们通常采用实时与失焦叠加的方式`input+blur`。
...
前面的示例只是用于展示Regular UI表单验证体系的使用方法，不一定符合实际需求。下面举几种比较合理的使用案例：

表单提交按钮按照在什么情况下可点击来分，常见的且比较合理的有以下几种情况：始终可点、必填项有内容可点、必填项内容正确可点。
...
let component = new RGUI.Component({
    ...
    computed: {
        canSubmit() {
            return Object.keys(this.data.rules).every((key) => this.data.rules[key].every((rule) => rule.success));
        },
    },
    submit: function() {
        if(!this.$get('canSubmit'))
